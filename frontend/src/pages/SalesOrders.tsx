...
import { CheckCircle2, Eye, Plus, Search, ShoppingBag, XCircle } from "lucide-react";
import { unwrapError } from "@/api/client";
import { fetchProducts } from "@/api/products";
import { createSalesOrder, fetchSalesOrders, updateSalesOrderStatus } from "@/api/sales";
import type { Product, SalesOrder } from "@/api/types";
...
export function SalesOrders() {
  ...
  // Queries
  const ordersQuery = useQuery({ queryKey: ["sales"], queryFn: fetchSalesOrders });
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => toast.error(unwrapError(err, "Could not create sales order")),
  });

  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => updateSalesOrderStatus(id, status),
    onSuccess: (updated) => {
  ...
  const handleProductSelect = (productId: string) => {
    ...
    if (prod) {
      setDraftPrice(prod.selling_price);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const avgValue = completed.length ? totalSales / completed.length : 0;
    const pending = orders.filter(o => o.status === "pending").length;

    return { totalSales, completed: completed.length, avgValue, pending };
  }, [ordersQuery.data]);
  ...
  const selectedProductStock = useMemo(() => {
    ...
    const prod = productsMap.get(Number(draftProduct));
    return prod ? prod.current_stock : null;
  }, [draftProduct, productsMap]);
  ...
  const columns: Column<SalesOrder>[] = [
    ...
    },
    {
      key: "customer_name",
      header: "Customer",
      render: (row) => <span className="font-medium text-zinc-900 dark:text-zinc-100">{row.customer_name}</span>,
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <div>
      <PageHeader
        title="Sales Orders"
        subtitle="Manage customer transactions and invoice stock deductions."
        actions={
      ...
      <DataTable
        ...
        empty={
          <EmptyState
            ...
            description="Create a new sales order to begin recording transactions."
          />
        }
      />
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={createMutation.isPending || !form.customer_name || form.items.length === 0}
            >
      ...
      >
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              <option value="pending">Draft (Pending Stock Deduction)</option>
              <option value="completed">Completed (Immediate Stock Deduction)</option>
            </Select>
          </div>

          <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3">
            <h3 className="text-[13px] font-medium text-zinc-800 dark:text-zinc-200 mb-2">Invoice Line Items</h3>
              ...
              >
                ...
                {(productsQuery.data ?? []).map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name} (SKU: {p.sku}) — {formatCurrency(p.selling_price)}
                  </option>
                ))}
              </Select>
            ...
            {selectedProductStock !== null && (
              <div className="mt-1.5 flex items-center gap-2 text-[12px]">
                <span className="text-zinc-500">Warehouse Available Stock:</span>
                <Badge tone={selectedProductStock >= draftQty ? "success" : "danger"} className="font-mono">
                  {selectedProductStock} units
                </Badge>
                {selectedProductStock < draftQty && (
                  <span className="text-red-500 font-medium animate-pulse">
                    ⚠️ Quantity exceeds available stock! Saving as "Completed" will fail.
                  </span>
                )}
              </div>
            )}
          ...
          <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-3">
            <span className="text-[13px] text-zinc-500">Order Subtotal:</span>
      ...
      <Modal
        ...
        title={selectedOrder ? `Sales Order Details - #${selectedOrder.id}` : ""}
        footer={
        ...
        {selectedOrder && (
          <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 p-3 surface rounded-card border border-zinc-100 dark:border-zinc-800">
              <div>
                <p className="text-[11px] uppercase tracking-wider text-zinc-400 font-semibold">Customer</p>
                <p className="text-[14px] font-semibold text-zinc-900 dark:text-zinc-100">{selectedOrder.customer_name}</p>
            ...
            <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-3 px-2">
              <span className="text-[14px] font-medium text-zinc-600 dark:text-zinc-400">Total Order Amount:</span>
